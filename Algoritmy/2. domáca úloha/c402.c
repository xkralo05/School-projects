...
**/
	bool identif = FALSE;
	tBTNodePtr iterator = (*RootPtr);
	tBTNodePtr help = NULL;	// Pomocná proměnná
	if((*RootPtr)== NULL)
	{
	...
	else
	{
		...
		{
			help = iterator;
			if(iterator->Cont > Content)
			{
				iterator = iterator->LPtr;
			}
			else if(iterator->Cont < Content)
			{
		...
		}while(iterator != NULL && identif != TRUE);
		...
		{
			tBTNodePtr help2 = malloc(sizeof(struct tBTNode));
			help2->Cont = Content;
			help2->LPtr = NULL;
			help2->RPtr = NULL;
...
**/
	tStackP helpStack;
	SInitP(&helpStack);
	Leftmost_Preorder(RootPtr,&helpStack);
	while(!SEmptyP(&helpStack))
	{
		RootPtr = STopPopP(&helpStack);
		Leftmost_Preorder(RootPtr->RPtr,&helpStack);
	}
	 //solved = FALSE;		  /* V případě řešení smažte tento řádek! */
}
...
**/
	tStackP helpStack;
	SInitP(&helpStack);
	Leftmost_Inorder(RootPtr,&helpStack);
	while(!SEmptyP(&helpStack))
	{
		RootPtr = STopPopP(&helpStack);
		BTWorkOut(RootPtr);
		Leftmost_Inorder(RootPtr->RPtr,&helpStack);
	}
...
**/
	...
	Leftmost_Postorder(RootPtr,&helpStackP,&helpStackB);

	while(!SEmptyP(&helpStackP))
	{
		RootPtr = STopPopP(&helpStackP);
		zleva = STopPopB(&helpStackB);
		if(zleva == TRUE)
		{
			SPushB(&helpStackB,FALSE);
			SPushP(&helpStackP,RootPtr);
			Leftmost_Postorder(RootPtr->RPtr,&helpStackP,&helpStackB);
		}
		else
		{
			BTWorkOut(RootPtr);
...
**/
	...
	}while((*RootPtr) !=NULL || !SEmptyP(&helpStack));
